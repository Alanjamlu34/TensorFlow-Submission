...
class myCallback(tf.keras.callbacks.Callback):
  def on_epoch_end(self, epoch, logs={}):
    '''
    Halts the training when the MAE falls below 0.2
    ...
    if(logs.get('mae') < 0.2):
      ...
      print("\nMAE is below 0.2 so cancelling training!")
...
def solution_B5():
    ...
    with open('daily-max-temperatures.csv') as csvfile:
        ...
        step = 0
        for row in reader:
            temps.append(float(row[1]))  # append temperature
            time_step.append(step)  # append time step
            step=step + 1
    ...
    model.compile(loss=tf.keras.losses.Huber(),
                  ...
                  metrics=["mae"])
    model.fit(train_set, epochs=150, callbacks=[callbacks])
    return model
